...
> ⚠️ **Scope / status:** this covers the **manual load/unload path** (`LOAD_MATERIAL` /
> `QUIT_MATERIAL` → `FILAMENT_RACK_SET_TEMP`). The **automatic tool-change / flush path** used during
> CFS material *changes* is handled by a *different* compiled module (`box_wrapper.so`,
> `BoxAction.get_flush_temp` / `get_flush_max_temp`) that sets its own purge temperature internally —
> this fix does **not** intercept that path yet. That work is in progress; see
